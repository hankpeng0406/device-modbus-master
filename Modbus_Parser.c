...
bool Modbus_General_Node_Parser(sensor_info_node_t *curNode, WISE_Sensor *sensors, int count)
{
	...
	if(str != NULL)
	{
		strcpy(sensors[count].handler, str);           // Handler Name
	}
	...
	if(str != NULL)
	{
		...
		{
			strcpy(sensors[count].devicename, "");
			strcpy(sensors[count].type, str);          // Platform

			str = strtok (NULL, delim);
			if(str != NULL)
			{
				strcpy(sensors[count].name, str);      // Parameter: Parity, Stopbit...
			}
		}
		else
		{
			strcpy(sensors[count].devicename, str);

			str = strtok (NULL, delim);
			if(str != NULL)
			{
				strcpy(sensors[count].type, str);      // Type: Descrete Input, Coil, Input Register, Holding Register
			}

			str = strtok (NULL, delim);
			if(str != NULL)
			{
				strcpy(sensors[count].name, str);      // Name: Register Name
			}
...
bool Modbus_Parser_Set_FormatCheck(sensor_info_node_t *curNode, bool *bVal, double *dVal, char *sVal)
{
	...
	else
	{
		...
		{
			if((sv->type == cJSON_String) || (sv->type == cJSON_Number))
			{
				printf("sv->type : %s\n", sv->valuestring);
				sv->type = cJSON_String;
				strcpy(sVal, sv->valuestring);
			}
			else
			{
